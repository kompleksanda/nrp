...
def nrp_random(filename, cost_ratio, generations, show_plot):
    read_dataset(filename)
    budget = cost_ratio * sum(costs)
    ...
    while i < generations:
        ...
        fitness = single_fitness(random_solution)
        if(fitness[1] < budget):
            i += 1
...
def nrp_single(filename, cost_ratio, evaluations, show_plot):
    read_dataset(filename)
    budget = cost_ratio * sum(costs)
    print("The budget is " + str(budget) + ".")
    ...
    problem.function = single_fitness
...
def nrp_multi(filename, cost_ratio, evaluations, show_plot):
    read_dataset(filename)
    budget = cost_ratio * sum(costs)
    print("The budget is " + str(budget) + ".")
    ...
    problem.constraints[0] = Constraint("<=", budget)
    problem.directions[0] = problem.MAXIMIZE
    problem.function = multi_fitness
    problem.types[:] = Integer(0, 1)
    algorithm = NSGAII(problem)
    algorithm.run(evaluations)
    if show_plot:
...
def compare(filename, cost_ratio, random, generations,
            single, single_evaluations, multi, multi_evaluations):
    import matplotlib.pyplot as plt
    xs = []
    ys = []
    if random:
        random_solutions = nrp_random(filename, cost_ratio, generations, 0)
        nondominated_random_solutions = multi_nondom(random_solutions)
    ...
    if single:
        single_solutions = nrp_single(filename, cost_ratio, single_evaluations, 0)
        nondominated_single_solutions = single_nondom(single_solutions)
    ...
    if multi:
        multi_solutions = nrp_multi(filename, cost_ratio, multi_evaluations, 0)
        nondominated_multi_solutions = multi_nondom(multi_solutions)
...
def read_dataset(filename):
    costs.clear()
    scores.clear()
    if(filename[0] == 'c') or mode == "classical":
       classical_read(filename)
    else:
       realistic_read(filename)

def classical_read(filename):
    profits = []
    values = {}
    weights = []
    f = open(filename, 'r')
...
def plot(solutions, budget, title):
    ...
    #print("nd", str(nondominated_solutions))
